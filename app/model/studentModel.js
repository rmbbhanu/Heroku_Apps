...
Student.updateById = function(id, student, result){
  sql.query("UPDATE students SET fname=?,lname=?,roll_no=?,department=?,email_id=?,semester=?,address=?,class_id=? WHERE s_id = ?", [student.fname,student.lname,student.roll_no,student.department,student.email_id,student.semester,student.address,student.class,id], function (err, res) {
          if(err) {
              console.log("error: ", err);
                result(null, err);
             }
